...
contract Domains is ERC721URIStorage {
  ...
  // Svg for storing metadata on-chain
  ...
  constructor(string memory _tld) payable ERC721("Beima Name Service", "BNS") {
    TLD = _tld;
  ...
  function price(string calldata _name) public pure returns(uint) {
    ...
    if (len == 3 ) {
      return 3 * 10 ** 17; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals).
    } else if (len == 4) {
      return 2 * 10 ** 17;
    } else {
      return 1 * 10 ** 17;
  ...
  function register(string calldata _name) public payable {
    ...
    require(domains[_name] == address(0), 'domain already exits');
    if(!valid(_name)) revert InvalidName(_name);


    uint DomainPrice = price(_name);
    require(msg.value > DomainPrice, "Amount paid too small for your domain name");
    ...
    string memory json = Base64.encode(
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',name,'", "description": "A domain on the beima name service", "image": "data:image/svg+xml;base64, ', Base64.encode(bytes(finalSvg)), '", "length": "', strLen, '"}')
  ...
  function setRecord(string calldata _name, string calldata _record) public {
    require(domains[_name] == msg.sender, "Caller must be registered");
  ...
  function getAllNames() public view returns(string[] memory) {
    console.log("Getting all names registered");
